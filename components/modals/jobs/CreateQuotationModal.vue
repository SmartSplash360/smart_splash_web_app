<template>
    <div  @click="toggleJobQuoteModal({ show: false })"
      ...
      <form
          @click.stop
          class="overflow-auto flex min-h-[500px] flex-col gap-8 rounded-md bg-white p-8 mx-auto lg:min-w-[550px] dark:bg-[#31353F] lg:mr-14">
        ...
        <div class="flex flex-col justify-between gap-5 bg-[#d4ecf4] p-4 rounded-md">
        ...
        <ul class="flex flex-col gap-5">
          <li class="flex justify-between items-center">
            ...
            <h6 class="heading__h6 text-gray-800">${{ totalPriceServices }}</h6>
          </li>
          <li class="flex justify-between items-center">
            <h6 class="heading__h6 text-gray-600">Products</h6>
            <h6 class="heading__h6 text-gray-800">${{ totalPriceProducts }}</h6>
          </li>
          <li class="flex justify-between items-center">
            <h6 class="heading__h6 text-gray-600">Chemical Readings</h6>
            <h6 class="heading__h6 text-gray-800">${{ totalPriceChems }}</h6>
          </li>
          <li class="flex justify-between items-center mt-6">
            <h2 class="heading__h2 text-gray-800">Total</h2>
            <h2 class="heading__h2 text-gray-800">${{ totalPrice }}</h2>
          </li>
        </ul>
...
const {    
  ...
  const email = ref(customerDetails.email);
  const address = ref();
  ...
  const totalPrice = computed(() => (totalPriceServices + totalPriceProducts + totalPriceChems).toFixed(2))
  ...
  const handleSendQuote = () => {
    confirm.require({
      ...
      icon: 'pi pi-check',
      accept: async () => {
        toggleJobQuoteModal({ show : false});
  ...
  onMounted(() => {
    const poolInfo = customerDetails.bodies_of_water?.filter(bodyOfWaterItem => bodyOfWaterItem.pool_id = newJobPayload.pool_id)[0];
