<template>
  <div
    class="alert-table card hidden lg:flex flex-col gap-10 dark:text-white"
    :class="[currentMode == 'dark' ? 'dark-mode' : '']"
  >
    <DataTable :value="alerts" dataKey="id">
      <template #empty> No alerts found. </template>
      <Column field="id" header="Alert" sortable></Column>
      ...
      <Column field="description" header="Address">
        <template #body="slotProps">
          {{
      ...
      <Column field="price" header="Technician">
        <template #body="slotProps">
          {{
