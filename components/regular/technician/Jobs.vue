      ...
      <Accordion v-else :activeIndex="0">
          ...
          <template #header>
            <div class="flex-between w-full dark:text-white">
              ...
              :class="[job?.status === 'scheduled' ? 'border-[#009F10] text-[#009F10] bg-[#CCF2E2]' : 'border-[#D42F24] text-[#D42F24] bg-[#F8B4B4]']"
              >{{ job?.status === 'scheduled' ? 'scheduled' : 'Unvailable'}}</span>
              <span class="ml-4"> <font-awesome-icon icon="ellipsis-vertical" /></span>
            </div>
          </template>
          <div class="flex flex-col dark:text-white bg-[#d4ecf4] dark:bg-[#1B2028]">
            <div class="flex-between px-4 py-2">
              ...
              <span class="text-xs flex-1 flex justify-start">{{job.start_date}}</span>
            </div>
            <div class="flex-between px-4 py-2">
              <span class="text-gray-500 span__element flex-1">Address</span>
              <span class="text-xs flex-1 flex justify-start">{{job.customer?.address[0]?.address_line1}}</span>
            </div>
            <div class="flex-between px-4 py-2">
              <span class="text-gray-500 span__element flex-1">Pool</span>
              <span class="text-xs flex-1 flex justify-start">{{job.body_of_water?.name}}</span>
            </div>
            <div class="flex-between px-4 py-2">
            ...
            <div class="flex-between px-4 py-2">
              <span class="text-gray-500 span__element flex-1">Service Level</span>
              <span class="text-xs flex-1 flex justify-start">{{job.body_of_water?.name}}</span>
            </div>
            ...
            />
              <Button
                ...
                text raised rounded
                class="!w-[35px] !h-[35px] !bg-white dark:!bg-[#31353F]"
                @click="props.editItem({ id: job.id, item: { ...job },mobileEdit : true })"
            />
            <Button
                icon="pi pi-trash"
                text raised rounded
