...
export const useTemplateStore = defineStore("template", {
    ...
    getters: {
        ...
        getTemplateById: (state) => (id: number | string) => {
            return state.templates.find((template:any) => template.id === id);
        },
    ...
    actions: {
        ...
        async fetchTemplate(id: number | string) {
            ...
            axios.defaults.headers.common['Authorization'] = `Bearer ${jwt}`;
            try {
                const res = await axios.get(`http://localhost:8000/api/v1/templates/${id}`);
        ...
        async createTemplate(templatePayload: any) {
            ...
            axios.defaults.headers.common['Authorization'] = `Bearer ${jwt}`;
            try {
                const res = await axios.post(`http://localhost:8000/api/v1/templates`, templatePayload);
                console.log(res)
        ...
        async updateTemplate(id: number | string, templatePayload: any) {
            ...
            axios.defaults.headers.common['Authorization'] = `Bearer ${jwt}`;
            try {
                const res = await axios.post(`http://localhost:8000/api/v1/templates/${id}`, templatePayload);
                if (!res.data.success) {
                    throw new Error(res.data.message);
                }
            } catch (error) {
        ...
        async deleteTemplate(templateId: number | string) {
            ...
            axios.defaults.headers.common['Authorization'] = `Bearer ${jwt}`;
            try {
                const res = await axios.delete(`http://localhost:8000/api/v1/templates/${templateId}`);
